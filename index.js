...
//endpoint for creating user
  app.post('/createuser',[
    ...
    async(req,res)=>{
        ...
        if(user){
            return res.status(400).json({error:"Sorrya user with this email already exist"})
        }
...
//get user details by jsonwebtoken & login credentials
...
//endpoint for updating user details through id & we must enter jwt token in headers
  app.put('/updatenote/:id',fetchuser,async(req,res)=>{
    ...
    // console.log(note.ObjectId.toString)
    if(!note){return res.status(404).send("Not found")}

    
    
  
    if(note._id.toString()!==req.user.id){  //this is for checking that right user is updating details or not
  ...
  app.delete('/deletenote/:id',fetchuser,async(req,res)=>{
    ...
    if(note._id.toString()!==req.user.id){  //this is for checking that right user is updating note or not
